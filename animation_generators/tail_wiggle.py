...
class TailWiggleGenerator(AnimGeneratorBase):
    WINDOW_NAME = "TailWiggleGeneratorWin"
    WINDOW_TITLE = "Tail Swing & Wiggle"
    CHAIN_INPUT = "twg_baseInput"
    ROWS_PARENT = "twg_rowsParent"
    ...
    def show(self):
        ...
        win = cmds.window(self.WINDOW_NAME, t=self.WINDOW_TITLE, sizeable=True)
        col = cmds.columnLayout(adj=True, rs=6)

        r = cmds.rowLayout(nc=3, adj=2)
        ...
        for lbl in ["Node", "RotX", "RotY", "RotZ",
                     "OffX", "OffY", "OffZ",
                     "X:Halves", "Y:Halves", "Z:Halves",
                     "X:IsSine", "Y:IsSine", "Z:IsSine", "Del"]:
        ...
        cmds.button(l="Close", c=lambda *_: cmds.deleteUI(self.WINDOW_NAME))
        cmds.setParent("..")
    ...
    def populate_rows(self):
        ...
        if not chain:
            return
        for node in chain:
            self.add_row(node, 0.0, 25.0, 0.0,
    ...
    def add_row(self, node, x_amp, y_amp, z_amp,
        ...
        x_amp_f = cmds.floatField(v=float(x_amp), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_X)
        y_amp_f = cmds.floatField(v=float(y_amp), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_Y)
        z_amp_f = cmds.floatField(v=float(z_amp), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_Z)
        x_off_f = cmds.floatField(v=float(off_x), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_X)
        y_off_f = cmds.floatField(v=float(off_y), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_Y)
        z_off_f = cmds.floatField(v=float(off_z), pre=2, minValue=-1e6, maxValue=1e6, bgc=self.COLOR_Z)
        x_halves_cb = cmds.checkBox(l='X Halves', v=bool(x_halves))
        y_halves_cb = cmds.checkBox(l='Y Halves', v=bool(y_halves))
        z_halves_cb = cmds.checkBox(l='Z Halves', v=bool(z_halves))
        x_sine_cb = cmds.checkBox(l='X IsSine', v=bool(x_sine))
        y_sine_cb = cmds.checkBox(l='Y IsSine', v=bool(y_sine))
        z_sine_cb = cmds.checkBox(l='Z IsSine', v=bool(z_sine))
    ...
    def _apply_settings(self, data):
        ...
        try:
            if "x" in mir:
                cmds.checkBox(self.mirror_x_cb, e=True, v=bool(mir["x"]))
            if "y" in mir:
                cmds.checkBox(self.mirror_y_cb, e=True, v=bool(mir["y"]))
            if "z" in mir:
                cmds.checkBox(self.mirror_z_cb, e=True, v=bool(mir["z"]))
        except Exception:
            pass
    ...
    def animate(self):
        if not self.node_rows:
            cmds.warning("No nodes to animate. Scan chain first.")
            return
        start, end = self.timeline_range()
        if (end - start) <= 0:
            cmds.warning("Invalid timeline length.")
            return
    ...
    @staticmethod
    def key_axis(node, attr, amp, start, end, halves=True, is_sine=False, offset=0.0):
        plug = f"{node}.{attr}"
        ...
        if halves:
            sk(start, A)
            sk(half, -abs(A) if A >= 0 else abs(A))
            sk(end, A)
        else:
            if is_sine:
                sk(start, -A); sk(q1, A); sk(half, -A); sk(q3, A); sk(end, -A)
            else:
                sk(start, 0.0); sk(q1, A); sk(half, 0.0); sk(q3, -A); sk(end, 0.0)
